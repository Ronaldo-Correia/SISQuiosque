...
import com.ifba.SISQuiosque.repository.RedefinicaoSenhaTokenRepository;
...
public class RedefinicaoSenhaController {
    ...
    private void enviarEmailRedefinicao(String email, String token) {
        try {
            ...
            String resetLink = "http://localhost:8088/redefinir-senha?token=" + token;

            helper.setTo(email);
            helper.setSubject("🔐 Redefinição de Senha - Sistema Clínico");
            helper.setFrom("redefinirsenhasystem6@gmail.com");

            String htmlMsg = """
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
                        <h2 style="color: #bf303c; text-align: center;">Redefinição de Senha</h2>
                        ...
                        <div style="text-align: center; margin: 25px 0;">
                            <a href="%s" style="display: inline-block; padding: 12px 24px;
                                background-color: #bf303c; color: white; text-decoration: none;
